...
export default function Offer() {
  ...
  const [alert, setAlert] = useState('')
    ...
    const fetchData=()=>{
        setLoading(true)
        axios.get(`http://localhost:5126/api/offer/getall`).then(res=>{
            setData(res.data)
        }).catch(eror=>{
            console.log(eror)
        })
    ...
    const Search=(e)=>{
        ...
        if(search==null || search==''){
            ...
            axios.get(`http://localhost:5126/api/offer/search/${search}`).then(res=>{
                console.log(res)
    ...
    const RemoveData=(id)=>{
        axios.delete(`http://localhost:5126/api/offer/${id}`,{
            headers:{
                        ...
                        <CTableBody>
                          {
                            data.map((item, index) => {
                              return <CTableRow>
                                <CTableHeaderCell >{item.nameAndSurname}</CTableHeaderCell>
                                ...
                                <CTableDataCell>
                                  ...
                                  <a href={`/Admin/sikayetler/${item.id}`}>
